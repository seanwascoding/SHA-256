...
public:
    void state();
    vector<bitset<8>> message_transfer(string);
    vector<bitset<32>> chunk(vector<bitset<8>>);
    void algorithm(vector<bitset<32>>);
    uint32_t calculate_x(vector<bitset<32>>, int);
    uint32_t calculate_y(vector<bitset<32>>, int);
    uint32_t summation_1(uint32_t);
    uint32_t summation_2(uint32_t);
    uint32_t choice(uint32_t, uint32_t, uint32_t);
    uint32_t majority(uint32_t, uint32_t, uint32_t);

private:
    unsigned int a_h_temp[8] = {0};
